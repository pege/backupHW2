...
public abstract class Ast {
	...
	public String toString() {
		return String.format("(%s)@%x", AstOneLine.toString(this), System.identityHashCode(this));
	}
	...
	/**
	 * A binary operation combining a left and right operand, such as "1+2" or
	 * "3*4"
	 */
	public static class BinaryOp extends LeftRightExpr {
		...
		public static enum BOp {
			...
			B_TIMES("*"), B_DIV("/"), B_MOD("%"), B_PLUS("+"), B_MINUS("-"), B_AND("&&"), B_OR("||"), B_EQUAL(
					"=="), B_NOT_EQUAL("!="), B_LESS_THAN(
			 ...
			 * AND/OR.
			 * 
			 * @return <code>true</code> iff <code>A op B == B op A</code> for
			 *         this operator.
			 */
	...
	/**
	 * Used in {@link MethodDecl} to group together declarations and method
	 * bodies.
	 */
	...
	public static class MethodDecl extends Decl {
		...
		public MethodDecl(String returnType, String name, List<Pair<String>> formalParams, Seq decls, Seq body) {
			this(returnType, name, Pair.unzipA(formalParams), Pair.unzipB(formalParams), decls, body);
		}

		public MethodDecl(String returnType, String name, List<String> argumentTypes, List<String> argumentNames,
				Seq decls, Seq body) {
			super(2);
